...
public class ApplicationForm {
    ...
    public void initializeFields(String firstName, String lastName, Date dateOfBirth, String gender, int marks, String bap, long bapPhone, String ma, long maPhone, String address) {
        ...
        bapField.setCaretColor(Color.WHITE);
        bapKaPhoneField.setCaretColor(Color.WHITE);
        maField.setCaretColor(Color.WHITE);
        maKaPhoneField.setCaretColor(Color.WHITE);
    ...
    public void addGap(JTextField textField){
        textField.setBorder(BorderFactory.createCompoundBorder(
                textField.getBorder(), // Keep existing border
                BorderFactory.createEmptyBorder(0, 10, 0, 0) // Add padding (top, left, bottom, right)
        ));
    }
    ...
    public void addComponentsToMainPanel() {
        ...
        Color fieldForeGround = new Color(137,150,200);
        ...
        for(JPanel p: getAllComponents(subPanel, new JPanel())){
            p.setBackground(new Color(32,32,32));
        }

        ArrayList<JTextField> arr = getAllComponents(subPanel, new JTextField());
        for(JTextField t: arr){
            ...
            if(t == addressField)
                t.setPreferredSize(new Dimension(1000, 35));
            if(t == firstNameField || t == lastNameField)
                t.setFont(new Font("Tahoma", Font.PLAIN, 24));
        }

    }

    <T> ArrayList<T>  getAllComponents(Container c, T find){
        ArrayList<T> arr = new ArrayList<>();
        for(Component comp: c.getComponents()){
            if( comp.getClass().equals(find.getClass())){
                arr.add((T)comp);
            }
            if(comp instanceof Container){
                arr.addAll(getAllComponents((Container)comp, find));
            }
        }
        return arr;
    }
    ...
    void save(){
        try{
            ...
            int marks =  Integer.parseInt(this.marksField.getText().trim());
            String fname = this.bapField.getText();
            long fno = Long.parseLong(this.bapKaPhoneField.getText().trim());
            String mname = this.maField.getText();
            long mno = Long.parseLong(this.maKaPhoneField.getText().trim());
